...
{
	// build temp pointer to traversve code
	Node<T> * tempPtr{ front};
	int move{ 0 };

		// if list is out side 
	if (index > count || index < 0) {
		cout << "Those parameters do not exsist." << endl;
		throw 1;
	}
	else {
			// Make index move forware to "Index TH" positon
			while (move != index ) {
...
void SinglyLinkedList<T>::insertAtIndex(const unsigned int index, const T& value)
{
	...
	if (front == nullptr) {
		...
		while (move != index ) {
			...
			// Check to see if Move is one less than move 
			if (move == index - 1 ) {
				NewNode->data = value;
...
void SinglyLinkedList<T>::deleteAtIndex(const unsigned int index)
{
	...
	// check to see if list is empty 
	if (index > count || index < 0) {
		cout << "There is nothing in that index." << endl;
		return ;
	} else if (index == 0) {
		// Delete single node and front
		...
		tempNode = front;
		while (move != index) {
			// Check to see if Move is one less than index
			if (move == index - 1) {
				tempNode = tempPtr->link;
				tempPtr->link = tempNode->link;
				break;	
			}
			tempPtr = tempPtr->link;
			move++;
...
//TODO: Complete this method
// Video lecture 9-25 at 5 min for help
template <typename T>
void SinglyLinkedList<T>::deleteAllInstances(const T& value)
{
	...
	Node<T> *curr{ front };
		...
		if (front == back) {
			// You have found a node to delete
				//found a single node to delete
			if (front == nullptr) {
				cout << "The list is empty" << endl;
				...
				// 2 or more nodes starting at front
				// at the end set curr == to nullptr to exit loop
				//if (curr->data == value) {
				...
				//}// End front IF 
				//else{	// begin to transition
					while (curr->data != value) {
						//loop for move down list
				...
				// after two or more check if it at front
					// then check at end
					// else it is at the middle
						//prev->link = curr->link
						// delete curr
						// set curr to prev->link
