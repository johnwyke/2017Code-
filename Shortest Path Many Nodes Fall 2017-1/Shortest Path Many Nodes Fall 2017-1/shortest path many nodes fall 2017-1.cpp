...
// This still needs all the work. 
int getWeight(int source, int dest) {
	
	for (int i = source; i < rows[source+1]; i++) {
		if (dest == columns[i]) {
...
void createCsrArrays() {
	...
	for (int i = 0; i < edges.size(); i++) {
		graphWeights[i] = edges.at(i).weight;
		columns[i] = edges.at(i).destNode;
	}
	
	// takes care of indexes 
	for (int i = 1; i < numNodes+1; i++) {
		count = 0;
		for (int j = 0; j < edges.size(); j++) {
			if (edges.at(j).sourceNode == i -1) {
				count++;
			}
		}
