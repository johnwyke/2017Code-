...
Iterator<T> Iterator<T>::operator++() {
	...
	if (currIter->forward == nullptr) {
		pastTheEnd = true;
		return *this;
	}
	else {
		currIter= currIter->forward;
		return *this;
	}
}
...
bool Iterator<T>::operator!=(const Iterator<T>& rhs)const {
	return !(this->currIter == rhs.currIter && this->pastTheEnd == rhs.pastTheEnd);
}


template <typename T>
Iterator<T> Iterator<T>::operator++(int) {
	Iterator<T> tempVal = *this;
	if (currIter == nullptr) {
		tempVal.pastTheEnd = true;
	}
	else {
		currIter = currIter->forward;
		return tempVal;
...
void ourReverse(T begin, T end) {
	...
	--end;
	while (beginPlusOne != end && begin !=end) {
